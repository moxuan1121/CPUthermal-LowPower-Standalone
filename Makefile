...
CPUthermalSettings_FILES = Settings/FRootListController.m Settings/CPUthermalMountListController.m
...
after-stage::
	$(ECHO_NOTHING)mkdir -p "$(THEOS_STAGING_DIR)/Library/PreferenceLoader/Preferences" "$(THEOS_STAGING_DIR)/usr/local/share/CPUthermal"$(ECHO_END)
	...
	$(ECHO_NOTHING)cp Tools/freqtest.sh "$(THEOS_STAGING_DIR)/usr/local/bin/CPUthermalFreqTest.sh"; chmod 0755 "$(THEOS_STAGING_DIR)/usr/local/bin/CPUthermalFreqTest.sh"$(ECHO_END)
