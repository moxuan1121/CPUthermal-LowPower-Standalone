...
此版本拦截已观察到的三类对象 CPU 等级与预算 setter，并在 `MitigationController` 更新后读取当前 CPU 等级，仅在低于 2 时主动收紧到 2；不覆盖系统更严格的等级。不同机型/iOS 版本可能不调用这些 selector；模式切换后的预算恢复也必须通过设备日志与实测确认。由于部分隐根终端没有 Apple 的 `log` 工具，0.5.2 起会在隐根 `/var/mobile/Library/Preferences/com.huayuarc.cputhermal.lowpower.status.txt` 写一份简短状态；在设备终端运行 `cat /var/mobile/Library/Preferences/com.huayuarc.cputhermal.lowpower.status.txt` 即可查看注入、配置和 CPU 对象命中情况。`level2Requested=1` 只表示插件请求了等级 2，不等于已证实物理频率变化。测试前不要用于依赖稳定散热或性能的设备。若要达到原包的全覆盖路径，需要进一步针对目标设备确认私有 API 和恢复流程。
